...
let temporizadorRutinario;

// 1. Variable para saber qué botón se presionó (Por defecto Entrada)
let tipoRegistroActual = "Entrada";

// 2. Capturamos los botones de tu index.html
const btnEntrada = document.querySelector(".btn-entrada");
const btnSalida = document.querySelector(".btn-salida");

// 3. Eventos para cambiar el tipo de QR cuando das clic
btnEntrada.addEventListener("click", () => {
  tipoRegistroActual = "Entrada";
  generarNuevoQR(); // Forzamos a que el QR cambie inmediatamente
  alert("Modo actualizado: Ahora se registrarán Entradas");
});

btnSalida.addEventListener("click", () => {
  tipoRegistroActual = "Salida";
  generarNuevoQR();
  alert("Modo actualizado: Ahora se registrarán Salidas");
});

// Función encargada de emitir un nuevo Token Limpio
async function generarNuevoQR() {
  const nuevoToken =
    Math.random().toString(36).substring(2) + Date.now().toString(36);

  // Inicializamos la ubicación mandando el "tipo" a Firebase
  await setDoc(doc(db, "monitores", UBICACION_ID), {
    tokenActivo: nuevoToken,
    status: "esperando",
    tipo: tipoRegistroActual, // <--- AQUÍ SE GUARDA SI ES ENTRADA O SALIDA
    quienEscaneo: "",
